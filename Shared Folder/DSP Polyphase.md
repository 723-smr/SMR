...
$$
仔细观察以上顺序（$h[n]$是关于$x_{(L)}[n]$的FIR冲激响应），$y[0]$~$y[7]$每个相位都相差一个插值后的间隔。$y[0]$、$y[8]$、$y[16]$...只和$h[0]$、$h[8]$、$h[16]$...有关，所以将$h[0]$、$h[8]$、$h[16]$...作为sub-FIR,依此类推。另外同理，$y[8]$~$y[15]$每个点相位相差一个采样点后的间隔。
