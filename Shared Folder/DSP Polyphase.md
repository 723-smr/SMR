...
$$
	f_{ser} = L \times f_{bb} 
$$
$y[0]$~$y[7]$每个序列都相差$\frac{1}{L\times f_{bb}}$，$y[8]$~$y[15]$同理，依此类推。$y[0]$ 和 $y[8]$ 相差的是$\frac{1}{f_{bb}}$。
***
>**e.g.**
...
3. 当$CLK_{ser}$的第一个上升沿到来时，Serializer输出 $y[0]$；
4. $CLK_{ser}$的第二个上升沿到来时，Serializer输出 $y[1]$；
5. ...

以此类推，$y[0]$~$y[7]$ 以3.2GHz的频率输出，等到第二个基带周期到来，$y[8]$~$y[15]$ 同样也以3.2GHz的频率输出。这样就获得了插零插值+低通滤波的双重效果。
***
