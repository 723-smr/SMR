...
$$
仔细观察以上顺序（$h[n]$是关于$x_{(L)}[n]$的FIR冲激响应），$y[0]$、$y[8]$、$y[16]$...只和$h[0]$、$h[8]$、$h[16]$...有关，所以将$h[0]$、$h[8]$、$h[16]$...作为sub-FIR,依此类推。

# 二、Polyphase的工程解释

那就可以得出结论，我们需要将FIR滤波器的冲激响应分成$L$组，以方便对应相移要求。另外每组前后两个冲激响应应该差L个样本点。

**上采样前后clock理解：**$y[0]$~$y[7]$每个相位都相差插零后一个的间隔，同理$y[8]$~$y[15]$每个点相位也相差插零后一个的间隔，依此类推。$y[0]$和$y[8]$之间相差的是插零前的一个间隔，即依次输入信号$x[k]$不同bit的间隔。
