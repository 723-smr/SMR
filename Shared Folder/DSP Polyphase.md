...
![](assets/DSP%20Polyphase/file-20260122155437892.png)
...
$$
	F(f) = E\tau Sinc(f\tau) 
$$
当原本的频谱中乘以一个sinc杂散后，高频的spurs就全部被显现出来了，根据通信要求，这些高频信号是不被允许存在的。因此我们就要想办法抑制这些高频信号。
...
![](assets/DSP%20Polyphase/file-20260122143958272.png)
$$
...
(1)式其实是插值加滤波后的最终结果，因此已经加入了串行器。但我想知道为什么要将滤波器分为$L$组，因此我需要排除串行器因素仅仅看FIR Filter的作用。假如说$N=160$，来举个例子看看，
$$
...
给出一组FIR的示意图像如下，图中相同颜色的为同一组sub-FIR。同时，需要特别关注的是FIR的图像是对称的，图中红色框出来的FIR的抽头系数是相同的（即$h[0]$=$h[63]$)，所以为了减少资源，在下面的操作中会进行简化。
...
***
**注意：** 
* 相邻$y[k]$间隔一个上采样周期，这种间隔实际在Serializer中完成（对应上图High-Speed Serializer中的$Z^{-1}$)，并不是在FIR中进行，在sub-FIR内$x[k]$与$h[k]$只是数值上进行的处理，并没有进行频率的提升！
* 这里的滤波效果体现在插零的地方有了幅度：
